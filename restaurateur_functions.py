...
def open_local_db_admin_connection(
    db_name: str = "restaurateur",
    username: str = "postgres",
    password: str = os.getenv("pg2_pass"),
):
    """Establish an admin connection to restaurateur."""

    connection = pg2.connect(database=db_name, user=username, password=password)
...
def create_source_tables(
    cursor,
    data_sources: tuple = (
        FOURSQUARE_LEGEND_DISCREPANCIES["Source"],
        YELP_LEGEND_DISCREPANCIES["Source"],
        GOOGLE_LEGEND_DISCREPANCIES["Source"],
    ),
) -> None:
    """
