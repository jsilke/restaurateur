...
def store_response_json(
    json_object: dict, file_name: str, directory: str = DIRECTORY
) -> None:
    """
...
def get_response_from_endpoint_and_store_json(endpoint_dict: dict) -> None:
    """
    ...
    """
    _json_object = get_json(
        url=endpoint_dict.get("endpoint"),
        parameters=endpoint_dict.get("params"),
        headers=endpoint_dict.get("headers"),
    )

    store_response_json(_json_object, file_name=endpoint_dict.get("file"))


def make_requests_and_store_json_responses(
    all_endpoint_dict: list[dict] = ALL_ENDPOINTS,
) -> None:
    """
    Takes in a list of endpoint_dicts and calls get_response_from_endpoint_and_store_json for each
    dictionary.
    """
    for endpoint_dictionary in all_endpoint_dict:
...
def get_next_page(next_page_token: str, google_endpoint_dict: dict = ALL_ENDPOINTS[-1]):
    """
    Takes in a next_page_token from a previous API search and uses it
    """
    json_object = get_json(
        url=google_endpoint_dict.get("endpoint"),
        parameters={
            "pagetoken": next_page_token,
            "key": google_endpoint_dict["params"]["key"],
        },
    )
