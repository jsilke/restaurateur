...
def get_response_from_endpoint_and_store_json(endpoint_dict: dict) -> None:
    ...
    """
    _json_object = get_json(url=endpoint_dict.get('endpoint'),
                            parameters=endpoint_dict.get('params'),
                            headers=endpoint_dict.get('headers'))

    store_response_json(_json_object,
                        file_name=endpoint_dict.get('file'))


def make_requests_and_store_json_responses(all_endpoint_dict: list[dict] = ALL_ENDPOINTS) -> None:
    """
...
def get_next_page(next_page_token: str, google_endpoint_dict: dict = ALL_ENDPOINTS[-1]):
    ...
    """
    json_object = get_json(url=google_endpoint_dict.get('endpoint'),
...
def main():
    ...
    # TODO automate Foursquare and Google pagination.
